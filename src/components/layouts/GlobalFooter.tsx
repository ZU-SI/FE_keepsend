"use client";

import { contactModalOpenAtom, policyModalOpenAtom } from "@/store/global-modal.store";
import { useSetAtom } from "jotai";

export default function GlobalFooter() {
  const setContactModalOpen = useSetAtom(contactModalOpenAtom);
  const setPolicyModalOpen = useSetAtom(policyModalOpenAtom);

  const handleMenuClick = (link: string) => (e: MouseEvent) => {
    e.preventDefault();
    if (!window) return;
    window.location.href = `${window.location.origin}/${link}`;
  };

  const handleContactClick = () => {
    setContactModalOpen(true);
  };

  return (
    <footer className="footer">
      {/* Main Footer Content */}
      <div className="footer__content">
        {/* Main Grid: Logo on left, right columns wrapper on right */}
        <div className="footer__main">
          {/* Left Section-Logo & Company Info */}
          <div className="footer__left">
            <div className="footer__logo">
              <span className="footer__logo-text">KEEPSEND</span>
            </div>
            <p className="footer__tagline">AI 기반 B2B 물류 플랫폼</p>
            <p className="footer__description">
              혁신적인 기술과 신뢰할 수 있는 서비스로 글로벌 물류의 미래를
              만들어갑니다.
            </p>
            <button onClick={handleContactClick} className="btn btn--primary btn--md mt-4">
              문의하기
            </button>
          </div>

          {/* Right columns wrapper-pushes to the right */}
          <div className="footer__right">
            {/* Mobile: 메뉴 and SNS in same row */}
            <div className="footer__columns">
              {/* Column 1-Menu */}
              <div className="footer__column">
                <h4 className="footer__column__title">메뉴</h4>
                <ul className="footer__menu">
                  <li className="footer__menu__item">
                    <button type="button" onClick={handleMenuClick("#service-intro")} className="footer__menu__link">
                      물류 서비스
                    </button>
                  </li>
                  <li className="footer__menu__item">
                    <button type="button" onClick={ handleMenuClick("#solution-intro")} className="footer__menu__link">
                      IT 솔루션
                    </button>
                  </li>
                  <li className="footer__menu__item">
                    <button type="button" onClick={handleMenuClick("news")} className="footer__menu__link">
                      소식
                    </button>
                  </li>
                </ul>
              </div>

              {/* Column 2-SNS */}
              <div className="footer__column">
                <h4 className="footer__column__title">SNS</h4>
                <ul className="footer__sns">
                  <li className="footer__sns__item">
                    <a href="#instagram" className="footer__sns__link">
                      <svg
                        className="footer__sns__icon"
                        fill="currentColor"
                        viewBox="0 0 24 24"
                      >
                        <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.015-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zM5.838 12a6.162 6.162 0 1 1 12.324 0 6.162 6.162 0 0 1-12.324 0zM12 16a4 4 0 1 1 0-8 4 4 0 0 1 0 8zm4.965-10.322a1.44 1.44 0 1 1 2.881.001 1.44 1.44 0 0 1-2.881-.001z" />
                      </svg>
                      Instagram
                    </a>
                  </li>
                  <li className="footer__sns__item">
                    <a href="#facebook" className="footer__sns__link">
                      <svg
                        className="footer__sns__icon"
                        fill="currentColor"
                        viewBox="0 0 24 24"
                      >
                        <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
                      </svg>
                      Facebook
                    </a>
                  </li>
                  <li className="footer__sns__item">
                    <a href="#youtube" className="footer__sns__link">
                      <svg
                        className="footer__sns__icon"
                        fill="currentColor"
                        viewBox="0 0 24 24"
                      >
                        <path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z" />
                      </svg>
                      YouTube
                    </a>
                  </li>
                </ul>
              </div>
            </div>

            {/* Column 3-Contact Info (stacks below on mobile) */}
            <div className="footer__column">
              <h4 className="footer__column__title">연락처</h4>
              <ul className="footer__contact">
                <li className="footer__contact__item">
                  <span className="footer__contact-label">Email:</span>
                  <br />
                  <a
                    href="mailto:contact@keepsend.com"
                    className="footer__contact__link"
                  >
                    contact@keepsend.com
                  </a>
                </li>
                <li className="footer__contact__item">
                  <span className="footer__contact-label">Phone:</span>
                  <br />
                  <a href="tel:0200000000" className="footer__contact__link">
                    02-xxxx-xxxx
                  </a>
                </li>
                <li className="footer__contact__item">
                  <span className="footer__contact-label">Address:</span>
                  <br />
                  서울특별시 강남구
                </li>
              </ul>
            </div>
          </div>
        </div>

        {/* Bottom Section-Copyright */}
        <div className="footer__bottom">
          <div className="footer__bottom__content">
            <p className="footer__copyright">
              © 2025 KEEPSEND. All rights reserved.
            </p>
            <div className="footer__legal">
              <button type="button" onClick={() => {setPolicyModalOpen(true)}} className="footer__legal__link">
                개인정보처리방침
              </button>
            </div>
          </div>
        </div>
      </div>
    </footer>
  );
}
